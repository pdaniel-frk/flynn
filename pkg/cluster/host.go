...
func (c *hostClient) PullSnapshot(receiveVolID string, sourceHostID string, sourceSnapID string) (*volume.Info, error) {
	...
	err := c.c.Post(fmt.Sprintf("/storage/volumes/%s/pullSnapshot", receiveVolID), pull, &res)
	return &res, err
}

func (c *hostClient) SendSnapshot(snapID string, assumeHaves []json.RawMessage) (io.ReadCloser, error) {
	res, err := c.c.RawReq("POST", fmt.Sprintf("/storage/volumes/%s/pullSnapshot", snapID), nil, assumeHaves, nil)
	if err != nil {
		return nil, err
	}
	return res.Body, nil
